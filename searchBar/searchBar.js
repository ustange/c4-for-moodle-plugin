...
// add origin
                ...
                //window.console.log('result handler',resultHandler)
                
                window.console.log('last query',lastQuery);
                //window.parent.postMessage({event: 'eexcess.queryTriggered', data: lastQuery},'*');
    ...
    return {
        ...
        init: function(tabs, config) {
            ...
            taglist.tagit({
                ...
                afterTagAdded: function(e, ui) {
                    ...
                    if (!util.preventQuery) {
                        util.queryUpdater();
                        sendHeightBar = bar[0].clientHeight;
            ...
            /*bar.keyup(function(e){
                if(e.keyCode == 13){
                    sendHeightBar = bar[0].clientHeight;
                    window.console.log('height after keyup',sendHeightBar);
                    window.parent.postMessage({event: 'eexcess.searchBarhei',data:sendHeightBar},'*');
                }
            });*/
            ...
            result_indicator = $('<a id="eexcess_result_indicator" href="#">16 results</a>').click(function(e) {
                window.console.log('heigth bar',bar[0]);
                sendHeightBar = bar[0].clientHeight;
                window.parent.postMessage({event: 'eexcess.searchBarhei',data:sendHeightBar},'*');
                e.preventDefault();
            ...
            if (settings.queryCrumbs.active) {
                ...
                qc.init(qc_div.get(0), function(query) {
                    ...
                    window.console.log('query',query.profile.contextKeywords);
                    sendHeightBar = bar[0].clientHeight;
                    window.parent.postMessage({event: 'eexcess.searchBarhei',data:sendHeightBar},'*');
                    window.parent.postMessage({event: 'eexcess.openResultsBar',data:""},'*');
                    if (!contentArea.is(':visible')) {
